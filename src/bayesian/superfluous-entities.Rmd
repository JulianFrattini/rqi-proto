...
```{r model-skew-comp}
m.no.exp <-
  brm(data = d, family = zero_inflated_poisson, f.no.exp, prior = priors.zip,
    iter = 4000, warmup = 1000, chains = 4, cores = 4,
    seed = 4, 
  )

m.no.dom <-
  brm(data = d, family = zero_inflated_poisson, f.no.dom, prior = priors.zip,
    iter = 4000, warmup = 1000, chains = 4, cores = 4,
    seed = 4
  )
```
...
param.m <- mcmc_intervals_data(m.zip)
