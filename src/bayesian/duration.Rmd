...
We assume that all independent variables affect the `duration` and that, additionally, the `experience` may affect the `domain.knowledge`, as a population with higher experience values has a higher likelihood of having acquired respective domain knowledge.
We currently do not assume other relationships.
For example, `education` and `experience` are assumed to be independent as gaining more experience does not affect the formal education level, and on the other hand, a specific formal education does not imply actual experience in SE or RE.
There may be a reason to assume that `experience` benefits `task.experience` but the authors did not see sufficient evidence to include this assumption in the DAG.
We invite scholars to challenge our beliefs and update this causal model, however
...
f.no.exp <- duration.scaled ~ 0 + 
               (1|PID)+ RQ*period +
               exp.re.scaled + edu + primary.role + 
               dom.os + dom.db + 
               model.train + model.occ 

f.no.dom <- duration.scaled ~ 0 + 
               (1|PID)+ RQ*period +
               exp.se.scaled + exp.re.scaled + edu + primary.role +
               model.train + model.occ 
```
