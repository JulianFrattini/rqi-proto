...
```{r requirements}
requirements <- read.csv(file=file.path(path_data, "rqi-objects.csv")) %>% 
  mutate(
...
```{r observations}
observations <- read.xlsx(file=file.path(path_data, "rqi-results.xlsx"), sheetName="Statistics") %>% 
  mutate(
    PID = as.numeric(PID),
...
demographics <- read.csv(file=file.path(path_data, "rqi-demographics.csv"))
...
d <- d %>% 
  mutate(
    ...
    associations.missing = Missing.association + Miswired.association,
